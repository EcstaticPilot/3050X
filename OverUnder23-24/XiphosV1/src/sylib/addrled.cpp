...
namespace sylib
{
    ...
    void Addrled::rainbow(int speed)
    {
        // less bright colors: #800000 and #800002, first letter can be changed to change the color
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        addrledControlMode = SylibAddrledControlModeMANUAL;
        gradient(0xff0000, 0xff0005, strip_length / 2 + 1, 0, false, true);
        gradient(0xff0000, 0xff0005, strip_length / 2, strip_length / 2 + 1, true, true);
        cycle(this->buffer, speed);
    }
