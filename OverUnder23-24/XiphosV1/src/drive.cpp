...
void PID::print()
{
	std::cout << error << "," << kp * error << "," << ki * totalError << "," << (kd * (prevError - error)) << "," << output << std::endl;
}
...
void inchDrive(double target, double speedMod, double timeout, double accuracy)
{
	...
	Brain.Timer.clear();
	// std::cout<<accuracy;
	float kp = 5.25, ki = 0.025, kd = 8.25;
	...
	{
		pid.error = (target) - ((LB.position(turns) * 2 * M_PI) * 1.625 * 3 / 5);
		if (pid.error < 5)
		{
			pid.kp = 7;
		}
...
{
	float min = fabs(camera.objects[0].centerX - 157.5);
	float output;
	float dist;
...
void turnToObject(vision::signature target, float angle, float accuracy, float tolerance, bool bigestObject)
{
	...
	while (true)
	{
		...
		if (pid.error < tolerance)
		{
			...
			pid.error = average(SMA);
			std::cout << "camera: " << pid.error << std::endl;
			pid = PID(0.5, 0, 1.5, pid.error, 20);
		}
		else
		{
...
void DriveToObject(vision::signature target, float dist, float accuracy)
{
	...
	PID cameraPID = PID(0.5, 0, 1.5, 0);
	std::vector<float> SMA = std::vector<float>(6);
	while (fabs(drivePID.error) > accuracy)
	{
		drivePID.error = (dist) - ((LB.position(turns) * 2 * M_PI) * 1.625 * 3 / 5);
		camera.takeSnapshot(target);
		wait(5,msec);
		SMA.push_back(distFromClosetObject());
		while (SMA.size() > 6)
		{
			SMA.erase(SMA.begin());
		}
		cameraPID.error = average(SMA);
		drivePID.update();
		cameraPID.update();
		float left = drivePID.output + cameraPID.output;
		float right = drivePID.output - cameraPID.output;
		drive(left,right,10);
	}
	drive_brake();
}
