...
void inchDrive(double target, double speedMod, double timeout, double accuracy)
{
	...
	{
		pid.error = (target) - ((LB.position(turns) * 2 * M_PI) * 1.375 * 3 / 4);
		if (pid.error < 5)
		{
			pid.kp = 7;
		}
...
void arcDrive(float finalAngle, float radius, bool isGlobal, bool isReverse, float accuracy)
{
	...
	while (fabs(pid.error) > accuracy)
	{
		...
		pid.error = dist - (avgPos * M_TWOPI * 1.375);					  // convert to inches using radians and radius
		pid.update();
		speed = pid.output;
		// pid.print();
...
void inchDriveMP(float target)
{
	...
	float vf = 10;
	float output = 1;
	SCurveProfile profile = SCurveProfile(target, a1, a2, j1, j2, v0, vf);
	LB.resetPosition();
	RB.resetPosition();
	float dist;
	while (output != 0)
	{
		dist =((LB.position(turns)+RB.position(turns) * M_PI) * 1.375 * 3 / 4);
		output = profile.getOutputDist(dist);
		drive(output,output,0);
		wait(10,msec);
	}
	drive_brake(hold);
}
...
void DriveToObject(vision::signature target, float dist, float accuracy)
{
	...
	{
		drivePID.error = (dist) - ((LB.position(turns) + RB.position(turns) * M_PI) * 1.375 * 3 / 5);
		camera.takeSnapshot(target);
		wait(5, msec);
