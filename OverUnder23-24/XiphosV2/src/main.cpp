...
bool devicesCheck()
{
	if (LF.installed() && RF.installed() && LB.installed() && RB.installed() && LM.installed() && RM.installed() &&
		gyro1.installed() && cata.installed() && intake.installed())
	{
		return true;
	}
	else
...
void usercontrol()
{
	...
	*/
	...
	while (true)
	{
		...
		//std::cout << cata.power() << "," << cata.position(degrees) << ", "<<cata.velocity(pct)<<","<<cata.efficiency()<< std::endl;
		...
		else if (Controller1.ButtonX.pressing() || cata.position(degrees) < 550)
		{
