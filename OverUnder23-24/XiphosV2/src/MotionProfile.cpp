 ...
 * @param vmax
 */
TrapezoidalProfile::TrapezoidalProfile(float target, float a1, float a2, float vmax) : target(target), a1(a1), a2(a2), vmax(vmax)
{
...
float TrapezoidalProfile::getOutput(float dist)
{
    ...
    {
        output = min(a1 * p1(dist), -a2 * (p3(dist) - p3(d2)) + vmax);
        // cout << a1 * p1(dist)<< "," << -a2 * (p3(dist) - p3(d2)) + vmax << endl;
    }
    else if (dist < d1)
    {
        output = a1 * p1(dist);
...
float SCurveProfile::p7(float t)
{
    ...
    float output = j2 * pow(t - timeMarks[7], 3) / 6 + p6(timeMarks[6]) + v6(timeMarks[6]) * (a) + -1 * a2 * pow(a, 2) / 2 + j2 * pow(a, 3) / 6 + vf * (t - timeMarks[7]);
    return output;
}
...
float SCurveProfile::tp2(float d)
{
    ...
    double numerator = sqrt((-2 * p1(t1) * a1) + v1(t1) * v1(t1) + 2 * a1 * d) + v1(t1) - a1 * t1 - 2 * v0;
    double denominator = a1;
    return (numerator / denominator) + tp1(distMarks[1]);
}
float SCurveProfile::tp3(float d)
{
    ...
    return expression/denominator + tp2(distMarks[2]);
}

float SCurveProfile::tp4(float d)
{
    float output = 1/vmax * d - distStart/vmax + tp3(distMarks[3]);
    return output;
}
float SCurveProfile::tp5(float d)
{
    float output = timeMarks[4];
    while(true){
        output = output - (p5(output)-d)/v5(output);
        if(fabs(p5(output)-d)<0.0001){
            break;
        }
    }
    return output;
...
{
    float output = timeMarks[5];
    while(true){
        output = output - (p6(output)-d)/v6(output);
        if(fabs(p6(output)-d)<0.0001){
            break;
        }
    }
    return output;
...
{
   float output = timeMarks[6];
    while(true){
        output = output - (p7(output)-d)/v7(output);
        if(fabs(p7(output)-d)<0.0001){
            break;
        }
    }
    return output;
 ...
 * @param vmax
 */
SCurveProfile::SCurveProfile(float target, float a1, float a2, float j1, float j2, float v0, float vf, float vmax) : target(target), a1(a1), a2(a2), j1(j1), j2(j2), v0(v0), vf(vf), vmax(vmax)
{
