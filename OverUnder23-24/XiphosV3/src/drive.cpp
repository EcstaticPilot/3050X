...
void inchDriveMP(float target)
{
	...
	while (output != 0)
	{
		...
		dist = ((LB.position(turns) * 3 / 4) * M_PI * 2 * radius);
		// std::cout<<dist<<std::endl;
		output = profile.getOutputDist(dist);
		std::cout << dist << "," << output << " ," << IPStoRPM(output) / 6 << " ," << LB.velocity(pct) << std::endl;
		drive(IPStoRPM(output) / 6, IPStoRPM(output) / 6, 10);
		// wait(10,msec);
	}
	drive_brake(hold);

	std::cout << "\n"
			  << LB.position(turns) << std::endl;
}
