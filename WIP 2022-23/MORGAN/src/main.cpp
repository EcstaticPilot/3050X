...
/*    Author:       NR 7700                                                   */
/*    Created:      July, 2022                                              */
/*    Description:  code of Nikhil Ramanuja , now owned by Ashley now owned by morgan                                 */
/*----------------------------------------------------------------------------*/
...
void xDrive(int ax1,int ax2,int ax4, int wt){
  // x drive code
  int Turn=ax4;
  int StrafeX=ax1;
  int DriveY=ax2;
...
//void inchDrive(float D, int V){ //D is distance in inches, function needs testing
  //LF.setVelocity(V, percent);
  //RF.setVelocity(V, percent);
...
//The side-strafing error is caused by X drive function or a hardware error, and is not a result of new changes
void inchDrive(float target, bool dir=true){ //dir=true is forward, dir=false is reverse
  target=fabs(target);
  float x=0.0;
...
void gyroTurn(bool dir, float A){ //dir=true is right turn, dir=false is left turn
  int FastSpeed=70;
  int Slowspeed=20;
...
void launch(){ //used for launching the disc with the piston when ready
  if(Controller1.ButtonR1.pressing()){
    Injector.set(true);
    wait(0.5,sec);
    Injector.set(false); //pushes disc into the wheel to launch and resets position
...
flywheelMonitor();
    ...
    // flywheel
   /* if(Controller1.ButtonR2.pressing()){ //refer to the variable definitions to change target speed
    if(abs(Fspeed-50)<20){
  ...
  if(Controller1.ButtonR2.pressing()){ //refer to the variable definitions to change speed
    targetSpeed=60;
  }
  if(Controller1.ButtonR1.pressing()){ //refer to the variable definitions to change speed
    targetSpeed=75;
  }
   ...
   if(Controller1.ButtonX.pressing()){
    ...
    xDrive(driveFactor*ax1,driveFactor*ax2,ax4,10);
