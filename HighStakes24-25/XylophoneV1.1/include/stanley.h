...
class stanleyEvent{
  ...
  public:
    float tval;
    stanleyEvent(void (*funcptr)(),float tval):tval(tval),func(funcptr){}
    void run(){
        func();
    }
};
...
{
public:
  static std::vector<stanleyEvent> stanleyEvents;
  ...
  static float slowdownStart;
public:
  static void setTimeout(float time);
  static void addEvent(void (*funcptr)(),float tval);
