...
class Addrled : private Device {
   ...
   public:
    ...
    void pulse(std::uint32_t color, int pulse_width, int speed = 1, int start_pos = 0,
               bool reverse = false, int end_pixel = -1, bool clear = true);
    ...
    /**
     * \brief graudually light a single LED
     * 
     * is blocking i think
     * 
     * \param pos 
     ...
     * 
     */
    void lightUp(int pos,std::uint32_t start_color, std::uint32_t end_color, int speed);

    /**
     * \brief strip light up the LED using gradients
     * 
     * \param color 
     * \param speed 
     */
    void stripLightUp(std::uint32_t color,int speed);

    void rainbow(int speed, bool reverse = false);
    void rainbow2(int speed, bool reverse = false);
    /**
     * \brief graudually light up the strip lineraly
     * 
     * \param color 
     * \param speed 
     */
    ...
    void gradualLightUp(std::uint32_t color,int speed);
    /**
     ...
     */
    static std::uint32_t hsv_to_hex(hsv in);
     ...
     * @param input 
     */
    void gridSet( std::vector< std::vector<uint32_t> > input);
