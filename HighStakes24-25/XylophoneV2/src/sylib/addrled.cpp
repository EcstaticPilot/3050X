...
namespace sylib
{
    ...
    std::vector<uint32_t> &Addrled::operator*() { return buffer; }
    ...
    void Addrled::update()
    {
        ...
        else if (addrledControlMode == SylibAddrledControlModeGRIDCYCLE)
        {
            for (std::size_t i = 0; i < buffer.size(); i++)
            {
                if (rotation_buffer[i] <= 0xFFFFFF)
                 ...
                 i++)
            {
                if (!cycleControlReversed)
                {
                    for(int i = 0; i<rotation_buffer.size();i++){
    ...
    sylib::SylibAddrledControlMode Addrled::getControlMode(){
        return addrledControlMode;
    }
    ...
    void Addrled::pulse(std::uint32_t color, int pulse_width, int speed, int start_pos, bool reverse,
                        int end_pixel, bool clear)
    {
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        sendingPulse = true;
        if (speed > 100)
                                           ...
                                           int fade_width)
    {
        ...
        step = fmin(fmax(step, 0), fade_width);
    ...
    std::uint32_t Addrled::interpolate_hsv(std::uint32_t start_color, std::uint32_t end_color, int step,
                                           int fade_width)
    {
        ...
        std::uint32_t solved = hsv_to_hex(interpolated);
                           ...
                           int start_pos, bool reverse, bool hsv_mode)
    {
        ...
        if (reverse)
        {
            std::uint32_t intermediate = start_color;
            start_color = end_color;
            end_color = intermediate;
        }
    ...
    void Addrled::lightUp(int pos, std::uint32_t start_color, std::uint32_t end_color, int speed)
    {
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        speed *= 1000;
        for (int i = 0; i < speed; i++)
        {
            set_pixel(interpolate_rgb(start_color, end_color, i, speed), pos);
            wait(10, msec);
        }
        addrledControlMode = SylibAddrledControlModeMANUAL;
    }
    /**
     * @brief light up strip
     *
     * @param color
     * @param speed
     */
    void Addrled::stripLightUp(std::uint32_t color, int speed)
    {

        for (int i = 0; i <= speed; i++)
        {
            gradient(interpolate_rgb(0, color, i, speed), 0);
            wait(5, msec);
        }
        for (int i = 0; i <= speed; i++)
        {
            gradient(color, interpolate_rgb(0, color, i, speed));
            wait(5, msec);
        }
    }

    void Addrled::gradualLightUp(std::uint32_t color, int speed)
    {
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        int step;
        // i represents the time interation
        ...
        for (int i = 0; i < strip_length; i++)
        {
            ...
            {
                step = fmin(fmax(i - pos, 0), strip_length);
                set_pixel(interpolate_rgb(0, color, step, strip_length), pos);
            }
            wait(speed, msec);
        }
        addrledControlMode = SylibAddrledControlModeMANUAL;
    ...
    void Addrled::rainbow(int speed, bool reverse)
    {
        // less bright colors: #800000 and #800002, first letter can be changed to change the color
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        uint32_t color1 = 0x4a0000;
        uint32_t color2 = 0x4a0001;
        addrledControlMode = SylibAddrledControlModeMANUAL;
        gradient(color1, color2, strip_length / 2 + 1, 0, reverse, true);
        gradient(color1, color2, strip_length / 2, strip_length / 2 + 1, !reverse, true);
        cycle(this->buffer, speed, 0 ,reverse);
    }

    void Addrled::rainbow2(int speed, bool reverse)
    {
        // less bright colors: #800000 and #800002, first letter can be changed to change the color
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        addrledControlMode = SylibAddrledControlModeMANUAL;
        gradient(0x4a0000, 0x4a0001, strip_length, 0, reverse, true);
        //gradient(0x800000, 0x800002, strip_length, 0, reverse, true);
        //gradient(0xff0000, 0xff0001, strip_length, 0, reverse, true);
        cycle(this->buffer, speed,0,reverse);
    }
    void Addrled::random()
    {
        mutex_lock _lock(sylib_port_mutexes[smart_port - 1]);
        addrledControlMode = SylibAddrledControlModeMANUAL;
        // set_all(rgb_to_hex(randomColor.r, randomColor.g, randomColor.b));
        for (int i = 0; i < strip_length; i++)
        {
            rgb randomColor;
            ...
            set_pixel(rgb_to_hex(randomColor.r, randomColor.g, randomColor.b), i);
        }
    }
    ...
    std::uint32_t Addrled::hsv_to_hex(hsv in)
    {
        double hh, p, q, t, ff;
        long i;
        rgb out;
    ...
    void Addrled::gridSet(std::vector<std::vector<uint32_t>> input)
    {
        ...
        for (int y = 0; y < input.size(); y++)
        {
            for (int x = 0; x < input[y].size(); x++)
            {
                ...
                output.push_back(input[y][x]);
            }
        }
        std::cout<<"done";
        this->set_buffer(output);
    }
    void Addrled::setPixel(int x, int y, uint32_t color)
    {
        int n;
        if (y % 2 == 1)
        {
        ...
        this->set_pixel(color, n);
    }

} // namespace sylib

std::vector<std::vector<uint32_t>> VectorToGrid(std::vector<uint32_t> list){
    std::vector<std::vector<uint32_t>> output;

    return output;
}
std::vector<uint32_t> GridToVector(std::vector<std::vector<uint32_t>> input){
    std::vector<uint32_t> output;
    output.reserve(64);
        for (int y = 0; y < input.size(); y++)
        {
